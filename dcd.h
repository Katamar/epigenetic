...
void *open_dcd_write (const char *path, const char *filetype,
  ...
  if (fio_open (path, FIO_WRITE, &fd) < 0) {
    printf("dcd) Could not open file %s for writing\n", path);
    return NULL;
  }
  ...
  nsavc  = step_traj;    /* number of timesteps between written DCD frames */
  delta  = dt;           /* length of a timestep                           */
  //nsavc  = simu_struct.step_traj;    /* number of timesteps between written DCD frames */
  //delta  = simu_struct.dt;           /* length of a timestep                           */

//  if (getenv("VMDDCDWRITEXPLORFORMAT") != NULL) {
//    with_unitcell = 0;      /* no unit cell info */
//    charmm = DCD_IS_XPLOR;  /* X-PLOR format */
//    printf("dcd) WARNING: Writing DCD file in X-PLOR format, \n");
//    printf("dcd) WARNING: unit cell information will be lost!\n");
//  } else {
...
////////////////////////////////////////////////////////////////////////
int write_timestep (void *v, double *box,
...
//  float *pos = ts->coords;
  double unitcell[6];
//  unitcell[0] = unitcell[2] = unitcell[4] = 1.0f;
//  unitcell[1] = unitcell[3] = unitcell[5] = 90.0f;

  /* copy atom coords into separate X/Y/Z arrays for writing */
  for (i=0; i<dcd->natoms; i++) {
    dcd->x[i] = (float) xcoord[i];
    dcd->y[i] = (float) ycoord[i];
    dcd->z[i] = (float) zcoord[i];
  }
  dcd->nsets++;
...
int read_dcdheader(fio_fd fd, int *N, int *NSET, int *ISTART,
  ...
  } else (*charmm) = DCD_IS_XPLOR; /* must be an X-PLOR format DCD file */
...
void *open_dcd_read (const char *path, const char *filetype,
   ...
   */
  {
    ...
    if (trjsize < 0) {
      printf("file '%s' appears to contain no timesteps.\n", path);
      fio_fclose(dcd->fd);
      free(dcd);
      return NULL;
    }
    ...
    if (dcd->nsets > 0 && newnsets != dcd->nsets)
      printf("Warning: DCD header claims %d frames, file size indicates there are actually %d frames\n", dcd->nsets, newnsets);
  ...
  if (!dcd->x || !dcd->y || !dcd->z) {
    printf("Unable to allocate space for %d atoms.\n", dcd->natoms);
    if (dcd->x)
      free(dcd->x);
    if (dcd->y)
      free(dcd->y);
...
/////////////////////////////////////////////////////////
int read_next_timestep (void *v, int natoms) {
